 ...
 * @author gbiz0
 */
@WebServlet(name = "ExcluirComputador", urlPatterns = {"/ExcluirComputador"})
public class ExcluirComputador extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try{
                ...
                if (dao.excluir(IdProduto)){
                    mensagem = "Computador excluido com sucesso!";
                }else{
                    mensagem = "Problemas ao excluir computadores!";
                }
                request.setAttribute("mensagem", mensagem);
                request.getRequestDispatcher("ListarComputador").forward(request, response);
            }catch(Exception e){
                System.out.println("Problemas na Servelet ao excluir o computador!"
                            + "Erro:"+ e.getMessage());
