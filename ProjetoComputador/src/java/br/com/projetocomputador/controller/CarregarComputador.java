    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
          int idComputador = Integer.parseInt(request.getParameter("idProduto"));
            ...
            try{
                GenericDAO daotipo = new TipoComputadorDAOImpl();
                request.setAttribute("tipocomputadores", daotipo.listar());
                
                GenericDAO dao = new ComputadorDAOImpl();
                request.setAttribute("computador", dao.carregar(idComputador));
                
                request.getRequestDispatcher("alterarcomputador.jsp").forward(request, response);
            }catch(Exception e){
            System.out.println("Problemas ao carregar computador! Erro:"+ e.getMessage());
            e.printStackTrace();
            }
        }
    }
